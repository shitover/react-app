...
const BookView = styled.span`
  flex: 1;
  text-align: center;
  color: ${({ view }) => (view === 'poor' ? 'red' : view === 'average' ? 'white' : 'green')};
`;
...
const booksReadData = [
  { name: 'Irrational Ecuberance - Robert J. Shiller', number: 1, view: 'excellent' },

  { name: 'Diplommacy - Henry Kissinger', number: 2, view: 'average' },

  { name: 'The Way to Wealth - Benjamin Franklin', number: 3, view: 'excellent' },

  { name: 'The Wealth of Nations - Adam Smith', number: 4, view: 'excellent' },

  { name: 'Dune - Frank Herbert', number: 5, view: 'average' },
  ...
  return (
    <BooksContainer>
      ...
      <AdviceContainer>
        ...
        {thankYouMessage && <p>{thankYouMessage}</p>} {/* Display thank you message */}
      </AdviceContainer>
    </BooksContainer>
  );
};
